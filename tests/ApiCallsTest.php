...
class ApiCallsTest extends TestCase
{
    ...
    public function setUp(): void
    {
        ...
        Config::set('raccomandate.base_uri', env('RACCOMANDATE_BASE_URI', 'bubu'));
        Config::set('raccomandate.api_key', env('RACCOMANDATE_API_KEY', 'bello'));
        ...
        chmod(self::$responseFile->getPathname(), 0666);
        chmod(self::$accettazioneFile->getPathname(), 0666);
        chmod(self::$archiviazioneFile->getPathname(), 0666);
    }
    ...
    public function testGetArchiviazioneFromId(): void
    {
        ...
        $dest2 = new DestinatarioPersonaItalia('Bubba', 'Gump', 'via', 'gamberi', '25', 'Mirandola', '41037', 'mo', 'italia');
        $data['destinatari'] = [$dest1, $dest2];
        $response = Raccomandate::createRaccomandata($data);
        $notValidId = $response['data'][0]['id'];
        $this->expectExceptionMessage('N. destinatari diverso da 1.');
